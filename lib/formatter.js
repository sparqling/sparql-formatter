...
const addPattern = (pattern) => {
  ...
  if (pattern.optional) {
    addLine('OPTIONAL {');
    addWhere(pattern.optional);
    addLine('}');
    return;
  }
  if (pattern.minus) {
    addLine('MINUS {');
    addWhere(pattern.minus);
    addLine('}');
    return;
  }
  if (pattern.filter) {
  ...
  if (pattern.service) {
    ...
    addLine(`SERVICE${silent}${getTripleElem(pattern.service)} {`);
    addGGPsub(pattern.pattern);
    addLine('}');
    return;
  }
...
const addFilter = (filter) => {
  if (filter.filter.notexists) {
    ...
    addWhere(filter.filter.notexists);
    addLine('}');
  } else if (filter.filter.exists) {
    addLine(`FILTER EXISTS {`);
    addWhere(filter.filter.exists);
    addLine('}');
  } else {
