...
exports.format = (syntaxTree, indentDepth = 2) => {
  ...
  if (syntaxTree.type === 'select') {
    addSelect(syntaxTree);
  } else if (syntaxTree.type === 'construct') {
    addConstruct(syntaxTree);
  } else if (syntaxTree.type === 'ask') {
    addAsk(syntaxTree);
  } else if (syntaxTree.type === 'describe') {
    addDescribe(syntaxTree);
...
const addSolutionModifier = (query) => {
  ...
  if (query.order) {
    addLine('ORDER BY ' + getOrderConditions(query.order));
  }
