...
const getExpression = (expr) => {
  switch (expr.expressionType) {
    ...
    case 'relationalexpression':
      let op1 = getExpression(expr.op1);
      let op2;
      if (Array.isArray(expr.op2)) {
        op2 = expr.op2.map(getTripleElem).join(', ');
        op2 = `(${op2})`;
      } else {
        op2 = getExpression(expr.op2);
      }
      return getBracketted(`${op1} ${expr.operator} ${op2}`, expr.bracketted);
    case 'conditionaland':
...
const getFactor = (factor) => {
  let out;
  if (factor.op1) {
    out = getExpression(factor.op1) + ' ' + getFactors(factor.ops);
  } else {
    out = getExpression(factor);
  }
  return getBracketted(out, factor.bracketted);
};
