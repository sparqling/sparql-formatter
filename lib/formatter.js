...
const addAsk = (query) => {
  addLine('ASK {');
  increaseIndent();
  query.where.patterns.forEach(addPattern);
  decreaseIndent();
  addLine('}');
...
const addUnit = (unit) => {
  if (unit.type === 'insertdata') {
    ...
    increaseIndent();
    unit.where.patterns.forEach(addPattern);
    decreaseIndent();
    addLine('}');
    ...
    if (unit.delete) {
      ...
      addLine('{');
      increaseIndent();
      addTriples(unit.delete.triples);
      decreaseIndent();
      addLine('}');
    }
    if (unit.insert) {
      ...
      addLine('{');
      increaseIndent();
      addTriples(unit.insert.triples);
      decreaseIndent();
      addLine('}');
    }
    if (unit.using) {
...
const addConstruct = (query) => {
  if (query.template) {
    ...
    increaseIndent();
    addTriples(query.template.triples);
    decreaseIndent();
    addLine('}');
  } else {
    addLine('CONSTRUCT');
