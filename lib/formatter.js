...
exports.format = (syntaxTree, indentDepth = 2) => {
  ...
  while (commentsList.length > 0) {
    output.push(commentsList.shift().text);
  }
...
const addLine = (lineText) => {
  output.push(currentIndent + lineText);
};

const addLineWithComment = (line, pos) => {
  let commentAdded = false;
  while (commentsList.length > 0 && commentsList[0].line < pos) {
...
const addSelect = (select) => {
  ...
  addLineWithComment(`SELECT ${args}`, lastLine);

  addDataset(select.dataset);

  addLineWithComment('WHERE {', lastLine + 1);
  addGroupGraphPatternSub(select.pattern);
  addLineWithComment('}', select.pattern.location.end.line);
