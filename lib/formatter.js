...
const addTriplePath = (triplepath) => {
  ...
  let props = '';
  let pos;
  const sLen = s ? s.length : 4;
  triplepath.propertylist.pairs.forEach((pair) => {
    const p = getTripleElem(pair[0]);
    const o = getTripleElem(pair[1]);
    if (props) {
      addLineWithComment(`${props} ;`, pos);
      props = ' '.repeat(sLen) + ` ${p} ${o}`;
      if (pair[0].location) {
        pos = pair[0].location.start.offset;
      } else {
        pos = pair[1][0].location.start.offset;
      }
    } else {
      props = `${s} ${p} ${o}`;
      pos = triplepath.chainSubject.location.start.offset;
    }
  });
  addLineWithComment(`${props} .`, pos);
};
