...
exports.format = (syntaxTree, indentDepth = 2) => {
  ...
  if (syntaxTree.body?.type === 'select') {
    addSelect(syntaxTree.body);
  } else if (syntaxTree.body?.type === 'construct') {
    addConstruct(syntaxTree.body);
  } else if (syntaxTree.body?.type === 'ask') {
    addAsk(syntaxTree.body);
  } else if (syntaxTree.body?.type === 'describe') {
...
const addAsk = (query) => {
  addLine('ASK {');
  increaseIndent();
  query.pattern.patterns.forEach(addPattern);
  decreaseIndent();
  addLine('}');
...
const addUnit = (unit) => {
  if (unit.type === 'insertdata') {
    ...
    increaseIndent();
    unit.pattern.patterns.forEach(addPattern);
    decreaseIndent();
    addLine('}');
...
const addSelect = (query) => {
  ...
  let args = '';
  if (query.modifier) {
    args += `${query.modifier.toString()} `;
  }
  args += proj.map(getProjection).join(' ');
  addLineWithComment(`SELECT ${args}`, pos);

  const datasetEndPos= addDataset(query.dataset);
  ...
  query.pattern.patterns.forEach(addPattern);
  decreaseIndent();
  addLineWithComment('}', query.pattern.location.end.offset);
...
const addConstruct = (query) => {
  if (query.template) {
    ...
    increaseIndent();
    addTriplesBlock(query.template.triplesblock);
    decreaseIndent();
    addLine('}');
  } else {
    addLine('CONSTRUCT');
  ...
  addLine('WHERE {');
  if (query.pattern.patterns) {
    increaseIndent();
    query.pattern.patterns.forEach(addPattern);
    decreaseIndent();
  } else {
    increaseIndent();
    addTriplesBlock(query.pattern.triplesblock);
    decreaseIndent();
  }
  addLine('}');
...
const addPattern = (pattern) => {
  if (pattern.patterns || pattern.type === 'select') {
    addGGP(pattern);
    return;
  }
  if (pattern.data) {
  ...
  switch (pattern.token) {
    ...
    case 'graphgraphpattern':
      ...
      increaseIndent();
      pattern.value.patterns.forEach(addPattern);
      decreaseIndent();
      addLine('}');
      break;
    case 'unionpattern':
...
const getProjection = (projection) => {
  switch (projection.kind) {
    case '*':
      return '*';
    case 'var':
      if (projection.value.varType === '$') {
        return '$' + projection.value.varName;
      } else {
        return '?' + projection.value.varName;
      }
    case 'aliased':
      return `(${getExpression(projection.expression)} AS ${getVar(projection.alias)})`;
    default:
