...
exports.format = (syntaxTree, indentDepth = 2) => {
  ...
  if (syntaxTree.query?.type === 'select') {
    addSelect(syntaxTree.query);
  } else if (syntaxTree.query?.type === 'construct') {
    addConstruct(syntaxTree.query);
  } else if (syntaxTree.query?.type === 'ask') {
    addAsk(syntaxTree.query);
  } else if (syntaxTree.query?.type === 'describe') {
    addDescribe(syntaxTree.query);
...
const addAsk = (query) => {
  addLine('ASK {');
  addGroupGraphPatternSub(query.pattern);
  addLine('}');
